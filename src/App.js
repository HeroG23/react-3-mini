...
class App extends Component {
  ...
  getPotentialBuyers() {
    ...
    axios.get('https://joes-autos.herokuapp.com/api/buyers').then(results=>{
      toast.success('Successfully found some potential buyers');
      this.setState({'buyersToDisplay': results.data});
    }).catch(() => toast.error("Failed to find a buyer"))
  }

  sellCar( id ) {
    //axios (DELETE)
    //setState with results --> vehiclesToDisplay
  ...
  addCar() {
    ...
    .then( results => {
     ...
     toast.error('Failed at adding new vehicle.') );
  }
  ...
  deleteBuyer(id) {
    ...
    .then(results => {
      ...
      toast.success("Buyer changed his mind");
    }).catch(() =>
